...
print_success_banner() {
  echo -e "${GREEN}╔═══════════════════════════════════════════════════════════════════════╗${NC}"
  echo -e "${GREEN}║                 AmeOS Installation Successful!                      ║${NC}"
  echo -e "${GREEN}║                                                                       ║${NC}"
...
print_failure_banner() {
  echo -e "${RED}╔═══════════════════════════════════════════════════════════════════════╗${NC}"
  echo -e "${RED}║                 AmeOS Installation Failed!                          ║${NC}"
  echo -e "${RED}║                                                                       ║${NC}"
...
print_header "Backup Existing AmeOS (if any)"

backupname=$(date +"%Y-%m-%d-%H-%M-%S")
if [ -d "ameos" ]; then
  echo -e "${GREEN}AmeOS exists, backing up to .config/ameos-backups folder.${NC}"
  if [ -d ".config/ameos-backups" ]; then
    echo -e "${GREEN}Moving current version of AmeOS to backups folder.${NC}"
    mv "$HOME"/ameos .config/ameos-backups/"$backupname"
    sleep 1
  else
    echo -e "${GREEN}Creating the backups folder & moving AmeOS to it.${NC}"
    mkdir -p .config/ameos-backups
    mv "$HOME"/ameos .config/ameos-backups/"$backupname"
    sleep 1
  fi
else
  echo -e "${GREEN}Thank you for choosing AmeOS.${NC}"
  echo -e "${GREEN}I hope you find your time here enjoyable!${NC}"
fi

print_header "Cloning AmeOS Repository"
git clone https://github.com/ametic/nixos-config.git --depth=1  ~/ameos
cd ~/ameos || exit 1
...
sudo nixos-rebuild boot --flake ~/ameos/#${profile}
